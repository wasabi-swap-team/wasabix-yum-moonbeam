...
import {LiquityVaultV2} from "./libraries/alchemist/LiquityVaultV2.sol";

import "hardhat/console.sol";

contract YumDaiVault is  ReentrancyGuard {
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumDaiVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumDaiVault: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumDaiVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumDaiVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumDaiVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumDaiVault: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumDaiVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumDaiVault: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumDaiVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumDaiVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumDaiVault: cannot initialize rewards address to 0x0");
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumDaiVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "YumDaiVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumDaiVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumDaiVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumDaiVault: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumDaiVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
